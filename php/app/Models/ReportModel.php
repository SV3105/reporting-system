...
class ReportModel
{
    ...
    private function solrGet(string $endpoint, array $params = []): array
    {
        ...
        if ($curlErr)
            throw new \RuntimeException("Solr connection error: $curlErr");
        if ($httpCode !== 200)
            throw new \RuntimeException("Solr returned HTTP $httpCode");
        ...
        foreach ($rows as $row) {
            ...
            elseif (!empty($row['search'])) {
                ...
                $escapedSubparts = array_map(function ($sub) use ($field) {
                    ...
                    if (str_ends_with($field, '_b'))
                        return $s; // Booleans don't need wildcards/quotes
                    return "*{$s}*";
                }, $subparts);
